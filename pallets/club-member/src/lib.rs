...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(10_000)]
		pub fn add_self(origin: OriginFor<T>) -> DispatchResult {
			// Check that the origin is signed.
			let who = ensure_signed(origin)?;

			// Check if the group has less than 2 members.
			let club_members = ClubMembers::<T>::get();
			ensure!(club_members.len() < 2, Error::<T>::CannotaddMember);

			// Check if the user is already a member.
			let location = club_members.binary_search(&who).err().ok_or(Error::<T>::AlreadyMember)?;

			// Add the user to the group.
			let mut updated_members = club_members.clone();
			updated_members.insert(location, who.clone());
			ClubMembers::<T>::put(&updated_members);
